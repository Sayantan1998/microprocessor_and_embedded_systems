...
void loop()
{ 
    /*setOnes(joystickXPin);
    setTens(joystickYPin);*/
    int x_data = analogRead(joystickXPin);
    int y_data = analogRead(joystickYPin);

    int temp = x_data;
    
    /*if((x_data >2000) && (y_data >2000) && (x_data <2100) && (y_data <2100))
      {
        setOnes(0);
        setTens(0);  
      }
    else if( ((x_data < 2000) || (x_data >2100)) && ( (y_data <= 2000) || ( y_data >2100) ) )
    {
        x_data = x_data/450;
        if(x_data > 9 & x_data <10)
          {x_data = 9;}
        y_data = y_data/450;
        if(y_data > 9 & y_data <10)
          {y_data = 9;}
